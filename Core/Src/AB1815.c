...
bool read(uint8_t offset, uint8_t *buf, uint8_t length)
{
	...
	// ab1815_status_e ret_code = ab1815_status_e_OK;
...
// 0x00
ab1815_status_e get_time(tmElements_t *time)
{
	ab1815_status_e to_ret = ab1815_status_e_ERROR;
...
enum ab1815_status_e get_oscillator_control(struct oscillator_control_t *oscillator_control)
{
	return read(AB1815_REG_OSCILLATOR_CONTROL, &oscillator_control->value, 1);
};

enum ab1815_status_e set_oscillator_control(struct oscillator_control_t *oscillator_control)
{
...
ab1815_status_e get_extension_ram(extension_ram_t *extension_ram)
{
